...
Purpose: AES encryption algorithm with OpenMP
Link: The code is modified by the link https://github.com/NihalHarish/ParallelAES-Encryption
*/
#include "AES.h"
...
void encrypt_p(char* chunk_input, char* chunk_output) {
    ...
    //DO OPERATIONS
    #pragma omp parallel shared(CHUNK_SIZE, chunk_input, chunk_output,nthreads) private(i,j,tid)
    {
...
void decrypt_p( char* chunk_output, char* chunk_oo) {
    ...
    #pragma omp parallel shared(CHUNK_SIZE, chunk_oo, chunk_output,) private(j,i,tid)
    {	
        tid = omp_get_thread_num();
        #pragma omp for schedule(dynamic)
        for(j=0; j<CHUNK_SIZE; j += 16) {
            unsigned char in[16], out[16];
            int block_no = j;
            memcpy(in, &chunk_output[block_no], 16);
            //The next function call decrypts the PlainText with the Key using AES algorithm.
		    for (i=0;i<keysize;i++){
				InvCipher(in, out);	
				memcpy(in,out,16);
			}
            memcpy(&chunk_oo[block_no], out, 16);
            memcpy(&chunk_oo[block_no], out, 16);
       }
   }
}
